...
Assemble a model from interchangeable parts — an AR(1) process on the log
reproduction number, coupled to a renewal infection process through a
generation interval, observed with negative-binomial error. `IDModel` also
...
draws of a 40-day infection trajectory:
...
![](assets/img/examples/model.png){fig-alt="Thirty prior predictive infection trajectories over 40 days on a log scale"}
